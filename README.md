...
| Area | Recommendation |
|---|---|
| Frontend | React or static HTML prototype first, then component migration |
| Backend | FastAPI for OAuth callbacks, indexing, and job orchestration |
| Queue | Celery, RQ, or a lightweight async worker |
| Database | PostgreSQL or SQLite during prototype stage |
...
Open `src/drive-fusion.html` in a browser to review the static product concept and layout. This prototype is suitable for converting into a React, Flask, or FastAPI powered implementation.

## Development phases

| Phase | Focus | Output |
|---|---|---|
| Phase 1 | Product prototype | Static web UI and documentation |
| Phase 2 | Authentication | Multi-account OAuth connection flow |
| Phase 3 | Indexing | Unified search and quota dashboard |
| Phase 4 | Transfer workflows | Copy queue, logs, and progress tracking |
| Phase 5 | Hardening | Security review, deployment, and telemetry |
...
git init
git add .
git commit -m "Initial commit: Drive Fusion project scaffold"
git branch -M main
git remote add origin https://github.com/YOUR_USERNAME/drive-fusion.git
